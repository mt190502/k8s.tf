...
resource "helm_release" "atlantis" {
  ...
  set = [
    ...
    { name = "defaultTFDistribution", value = "opentofu" },
    { name = "extraPath", value = "/plugins" },
    { name = "githubApp.id", value = try(var.secrets.app.github_app_id, "") },
    { name = "githubApp.installationId", value = try(var.secrets.app.github_installation_id, "") },
    { name = "githubApp.slug", value = var.config.github_app_slug },
    { name = "ingress.enabled", value = false },
    { name = "initConfig.containerSecurityContext.runAsUser", value = 0 },
    { name = "initConfig.enabled", value = true },
    { name = "initConfig.image", value = "debian:bookworm-slim" },
    { name = "initConfig.imagePullPolicy", value = "IfNotPresent" },
    { name = "initConfig.sharedDir", value = "/plugins" },
    { name = "initConfig.sharedDirReadOnly", value = true },
    { name = "initConfig.sizeLimit", value = "1Gi" },
    { name = "initConfig.workDir", value = "/tmp" },
    { name = "loadEnvFromSecrets[0]", value = "${var.config.name}-secrets" },
    ...
    { name = "route.ui.kind", value = "HTTPRoute" },
    { name = "route.ui.matches[0].path.type", value = "PathPrefix" },
    { name = "route.ui.matches[0].path.value", value = "/" },
    { name = "route.ui.parentRefs[0].name", value = var.config.gateway_name },
    { name = "route.ui.parentRefs[0].namespace", value = var.config.gateway_namespace },
    { name = "route.webhook.apiVersion", value = "gateway.networking.k8s.io/v1" },
    { name = "route.webhook.enabled", value = true },
    { name = "route.webhook.hostnames[0]", value = "${var.config.hostname}.${var.config.domain}" },
    { name = "route.webhook.kind", value = "HTTPRoute" },
    { name = "route.webhook.matches[0].path.type", value = "PathPrefix" },
    { name = "route.webhook.matches[0].path.value", value = "/events" },
    { name = "route.webhook.parentRefs[0].name", value = var.config.gateway_name },
    { name = "route.webhook.parentRefs[0].namespace", value = var.config.gateway_namespace },
    { name = "service.port", value = 80 },
    { name = "service.portName", value = "atlantis" },
    { name = "service.targetPort", value = 4141 },
    { name = "service.type", value = "ClusterIP" },
    { name = "serviceAccount.create", value = true },
    { name = "serviceAccount.mount", value = true },
