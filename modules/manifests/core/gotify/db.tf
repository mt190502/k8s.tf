...
#  Gotify uses a single connection string env var (GOTIFY_DATABASE_CONNECTION), so we             #
#  also store the assembled connection string in the secret under the "connection" key.           #
## ============================================================================================= ##
resource "kubernetes_secret_v1" "postgres" {
  ...
  data = {
    ...
    connection = "host=${var.config.name}-postgres-rw.${kubernetes_namespace_v1.this[0].metadata[0].name}.svc.cluster.local port=5432 user=${var.config.name} dbname=${var.config.name} password=${try(var.secrets.pg.password, "changeme")} sslmode=disable"
  }
  type       = "Opaque"
